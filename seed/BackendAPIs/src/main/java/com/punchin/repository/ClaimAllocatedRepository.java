...
public interface ClaimAllocatedRepository extends JpaRepository<ClaimAllocated, Long> {
    ...
    @Query(nativeQuery = true, value = "SELECT count(ca.*) FROM claim_allocated as ca INNER JOIN claims_data as cd ON ca.claims_data_id = cd.id WHERE cd.claim_status=:claimStatus AND ca.user_id =:userId")
