...
public interface DocumentUrlsRepository extends JpaRepository<DocumentUrls, Long> {
    ...
    @Query(nativeQuery = true, value = "select * from document_urls du inner join claim_documents_document_urls cddu ON du.id =cddu.document_urls_id where cddu.claim_documents_id in (SELECT id FROM claim_documents WHERE claims_data_id=37 AND upload_side_by='agent' AND is_active=true AND agent_doc_type='NOMINEE_KYC_PROOF')")
    List<DocumentUrls> findAllDocumentAccordingToClaimDocuments(Long id, String agent, boolean b, String docTypes);
}
